...
const PaymentModal: React.FC<PaymentModalProps> = ({ open, onClose }) => {
  ...
  const handleCurrencyChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    ...
    // Para birimi değiştiğinde, o para birimine ait ilk hesabı seç
      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers ref={dialogContentRef}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <Typography variant="subtitle2" gutterBottom>Plasiyer</Typography>
                            <TextField select fullWidth defaultValue="" variant="outlined" size="small" SelectProps={{ displayEmpty: true }}>
                <MenuItem value="" disabled>Seçiniz</MenuItem>
                <MenuItem value="plasiyer-1">Plasiyer 1</MenuItem>
                <MenuItem value="plasiyer-2">Plasiyer 2</MenuItem>
              </TextField>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography variant="subtitle2" gutterBottom>İşlem Tipi</Typography>
              <TextField select fullWidth defaultValue="tacir" variant="outlined" size="small">
                <MenuItem value="tacir">Tacir</MenuItem>
                <MenuItem value="personel">Personel</MenuItem>
              </TextField>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography variant="subtitle2" gutterBottom>İşlem Tarihi</Typography>
              <DatePicker
                value={selectedDate}
